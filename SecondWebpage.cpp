...
void readPumpLogFiles(
  ...
  for (int i = 1; i <= 10; i++) {
    Serial.printf("Processing Pump %d\n", i);

    // Daily log
    String dailyFileName = "/Pump_Logs/pump" + String(i) + "_Daily.txt";
    File dailyFile = openLogFile(dailyFileName, "r");
    if (dailyFile) {
    ...
    File monthlyFile = openLogFile(monthlyFileName, "r");
    if (monthlyFile) {
    ...
    File yearlyFile = openLogFile(yearlyFileName, "r");
    if (yearlyFile) {
