...
AsyncWebServer074 : Added ESP32-targz library and enabled compressed directory downloads from third webpage.Fuck miniz library, not compatable.
...
AsyncWebServer076 : discovered temp.tar.gz files created when compressing directories for download to web clients were being left on the file system even after successfully downloading to the web client. created delete task to run 5 seconds after sending file to webclient.
...
AsyncWebServer078 : added tgzProducer task into MonitorStacks() to verify adequate stack size for downloading .tar compressed directories. Updated monitorStacks function to properly display stack size memory usage.

AsyncWebServer079 : tgzProducer task completing to quick to be captured by monitorStacks. tgzProducer stack usage now cached and picked up by monitorStacks also taskmonitorStacks is no longer pined to core 1.
...
Library Update : ESP Async WebServer v3.9.4 > 3.9.5. ESP32-targz V1.2.9 > 1.3.0. esp322 v3.3.5 > 3.3.6
...
ESP - Massive update for downoading through the Webpage. Archive size estimate before download. Directory downloads now call: /fs/download_compressed/info?dir=/Some_Folder It estimates source bytes, file count, folder count, and whether the estimate was truncated. User warning before archive download Pump Logs, Temperature Logs, and the full Flash Memory Browser now show a confirmation dialog with: Source size, File count Folder count, Estimated download time, Archive status polling, The browser now polls: /fs/download_compressed/status It shows archive state, source size, bytes sent, bytes produced, and pending pump-log RAM events. Stop Download button Added: /fs/download_compressed/cancel The Pump Logs browser, Temperature Logs browser, and full Flash Memory Browser now show a Stop Download button while an archive is active. Scaled TGZ no-data timeout The previous fixed 30-second timeout is now scaled from the estimated source size and file count, capped at 120 seconds. Important note: this phase still uses the current ESP32-targz producer, so Stop Download stops the browser-side stream and requests cancellation, but the producer may still need to drain safely in the background because we are still protecting against the ESP32-targz abort behavior. Found AsyncWebServer090_ESP_IDF_V5 works when downloading the Temperature_Logs but every version after that crashes even though the smaller directories will succeed. Confirmed this to be true with 10 successfull downloads in a row and no failures. it seems that the failures happen with the tgz compressor and there was a change in version AsyncWebServer090_ESP_IDF_V5 where the compressor line " TarGzPacker::compress(&tarGzFS, s->srcPath.c_str(), &s->stream); " was changes to " TarGzPacker::compress(&LittleFS, safeSrc.c_str(), out, nullptr); ". Reverted this change and download still failed with abort on core 0 "21:14:16.444 -> [TGZ] Phase2G info request: dir=/Temperature_Logs. 21:14:29.452 -> [TGZ] Phase2G download request: dir=/Temperature_Logs21:14:35.119 -> [TGZ] request ring=262144 bytes (PSRAM), cache_location=1. 21:14:35.160 -> [TGZ] heap free=40724 lfb=14324 | psram free=8378364 lfb=8257524 . 21:14:35.160 -> [TGZ] producer start: src=/Temperature_Logs/. 21:14:35.160 -> [TGZ] producer compress begin. 21:14:38.249 -> 21:14:38.313 -> abort() was called at PC 0x4037729b on core 0. 21:14:38.313 -> 21:14:38.313 -> Backtrace: 0x4037e899:0x3fcd6c10 0x4037e861:0x3fcd6c30 0x40385621:0x3fcd6c50 0x4037729b:0x3fcd6cd0 0x40377472:0x3fcd6cf0 0x420d3e5b:0x3fcd6d10 0x420d3f23:0x3fcd6d30 0x420d3fcd:0x3fcd6d60 0x42024f05:0x3fcd6d80 0x4202515d:0x3fcd6da0 0x420252cb:0x3fcd6de0 0x420248a5:0x3fcd6e20 0x4202f92a:0x3fcd6e50 0x4202f557:0x3fcd6e90 0x4202f726:0x3fcd6f10 0x420304bf:0x3fcd6fa0 0x4203068f:0x3fcd7060 0x42012dff:0x3fcd70b0 0x4037f755:0x3fcd7100  ELF file SHA256: a301f5a33. 21:14:38.346 ->  21:14:38.840 -> Rebooting..." 
