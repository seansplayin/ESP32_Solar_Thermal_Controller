...
ESP32_Solar_Thermal_Controller_20260416060958 - Continued troubleshooting Network dropouts, lowered spi bus to 6mhz and increased NetworkRecoveryTask from 4096 to 8192 block size Made changes to DS18B20.cpp, in addition loading sensors a bus search is also performed to report any missing sensors against the sensor address list. Found TGZ fallback still attempting to allot 4MB for PSRAM ring when PSRAM selected and PSRAM is not available. Updated TGZ.cpp so even when PSRAM is selected but PSRAM is not available TGZ ring buffer falls back to smaller internal heap size and logs to Serial Log and HTTP and shows Requested bytes, chosen location, largest free block, PRAM largest free block. 



I have NOT replaced my current NetworkManager.cpp file with the version you provided in your last post yet because inline with the SPI clock tuning I continued to test lower and lower SPI clock frequencies and it seems the lower we go with SPI clock frequency it does not really increase the Network Adapter stability. I measured stability by determining the elapsed time between bootup and the first Ethernet Disconnected/webSocket client disconnected (id=1) event.  I do not know why but I can tell you this is what I observed. at 30 mhz spi frequency the first "WebSocket client disconnected (id=1)" event occurred after 17 minutes of runtime. I lowered the spi frequency to 25 mhz and the first webSocket client disconnected (id=1) event occurred 24 minutes after boot.  I lowered the SPI Frequency to 20 mhz and the first disconnect event after boot occurred at just 8 minutes after bootup. I lowered the SPI Frequency to 15 MHZ and for some reason the webpage would not even connect until I pushed the reset button on the ESP32 and the first disconnect event occurred at just 12 minutes after bootup and caused the ESP to completely crash (see serial monitor output below. At 10 Mhz SPI Frequency the first  "WebSocket client disconnected (id=1)" event occurred at just 16 minutes after bootup. At SPI Frequnecy of 8 mhz the the first At "WebSocket client disconnected (id=1)" event occurred at just 9 minutes after bootup. and when I reloaded the webpage the ethernet disconnected again and the Pump Runtimes iframe window actually duplicated messed up the second webpage data contained inside the Iframe. This may have been caused due to the disconnect occurring while the Iframe windows was loading and resizing the content. There does appear to be something off with this iframe regarding the scaling because sometimes when the iframe loads the content it is oversized and sometimes it is regular sized and basically matches the rest of the webpage character sizes.    At SPI Frequency of 6 MHZ the first "WebSocket client disconnected (id=1)" event occurred at just 2 minutes after bootup. At SPI Frequency of 4 MHZ I got up and left the computer but when I returned it had dropped the Ethernet connection 12 times in 5 hours although the first "WebSocket client disconnected (id=1)" event did not occur for 60 minutes since bootup. I ran this test again and this time with me sitting at the computer with the SPI bus at 4 MHZ and only 9 minutes elapsed until the first "WebSocket client disconnected (id=1)" event occurred with me sitting at the computer. At SPI Frequency 3 the first "WesSocket client disconnected (id=1)" occurred at 7 minutes. At SPI Frequency of just 2 Mhz the Iframe for the Temperature will not even load and that part of the webpage is completely blank. Also the Ethernet Adapter did not even make it 3 minutes after bootup before the Ethernet connection dropped. Oddly enough even though more than 2 minutes had elapsed since boot it doesn't appear the first webpage even connected until after the Ethernet Reconnected happened.   "01:51:56.879 -> WebSocket client disconnected (id=1) " 01:51:56.879 -> WebSocket client disconnected (id=1) 
01:51:56.879 -> [Network] Ethernet Disconnected
01:51:58.860 -> [Network] Ethernet Connected
01:52:28.863 -> [Network] Debounced recovery: restarting W5500 / ETH stack
01:52:28.863 -> [Network] Ethernet Disconnected
01:52:28.863 -> [Network] Ethernet Stopped
01:52:28.863 -> CORRUPT HEAP: Bad tail at 0x3fcd4d34. Expected 0xbaad5678 got 0xbaad5602
01:52:28.896 -> 
01:52:28.928 -> assert failed: multi_heap_free multi_heap_poisoning.c:279 (head != NULL)
01:52:28.928 -> 
01:52:28.928 -> 
01:52:28.928 -> Backtrace: 0x4037e899:0x3fcca720 0x4037e861:0x3fcca740 0x4038571a:0x3fcca760 0x40384337:0x3fcca8a0 0x4037669b:0x3fcca8c0 0x403857b9:0x3fcca8e0 0x42061df2:0x3fcca900 0x420db5ab:0x3fcca920 0x4204afec:0x3fcca940 0x42061b39:0x3fcca960 0x42061b61:0x3fcca980 0x4206284b:0x3fcca9c0 0x4202c735:0x3fcca9e0 0x4202afcc:0x3fccaa00 0x4200d57d:0x3fccaa20 0x4037f755:0x3fccaa90
01:52:28.961 -> 
01:52:28.961 -> 
01:52:28.961 -> 
01:52:28.961 -> 
01:52:28.961 -> ELF file SHA256: 4f8d03a86
01:52:28.961 -> 
01:52:29.325 -> Rebooting..."   
on the surface it appears that  4 mhz spi bus is the most stable but in the past if I wasn't sitting at my computer it would make it 8 hours without any Ethernet disconnect events occurring if I got up and left the computer.  After carefully monitoring the Ethernet drop outs while running all these tests one thing is absolutely clear to me. Me being present at my computer test bench is at least partially responsible for the Ethernet Disconnects events. Nearly every time I leave the area the disconnects become much left common compared to how often they occur when I'm sitting at my computer desk. A lot of the time as soon as I sit down and unlock my computer a Ethernet disconnect happens even though there might not have been a disconnected for the last hour. I suspect that Static Electricity is the culprit here. 8 days ago the ESP32 went 10 full hours while I was gone all day without a single Ethernet Disconnect event occurring until I sat down at the computer/test bench and i was using a 30 mhz spi bus frequency on that test. Also we can see in the data that Static Electricity is likely not the only root cause going on with the Ethernet Disconnects at least at certain test conditions because disconnects have happened when the computer was not in use like what happened last night with the SPI bus at only 4 MHZ. Several times in the past 30 MHZ appeared to me the most stable and it might be however I still to wonder if we have some other blocking event occurring that occasionally happens at the wrong time on the same core at the Ethernet Adapter and I would like to validate that no other functions are able to send out WS messages without the messages going through the ring queue scheduler. This was the single change we previously implemented that made an immediately and obvious stability increase that for the most part basically made the Ethernet Disconnects no longer occur at least when the system was left unattended for long periods of time around 10 hours. A potential clue of one possible problem is that at some point the " [WS] Received message: ping " stop appearing in the Serial Monitor. up to this point we've basically paid this observation very little attention and but there must be a reason this is happening since we created the code requesting the webpage to send these message about every 30 seconds I believe. When left unattended I've seen these messages continue for around 30 minutes and I do acknowledge that browser energy saving could somehow be at play here plenty of times it has stopped when the computer was not left unattended. It might be working evaluating to option of introducing some webpage code that will automatically refresh the webpage when this message is not sent out on schedule but I'm not certain of what we could learn from this but there must be a reason why the messages do not continue indefinitely. 
...
I have not implemented the instructions from your last post yet and I wanted to show you the edge case I just stumbled onto which installed a new RCT that has never been time sync'd with the network cable unplugged. Basically the serial log shows wdt reboots occur because it keeps trying to reconnect the ethernet adapter. Also I was thinking it would be useful during initial setup or when the RTC battery fails and internet is not available the system will continue to load and all functiosn will work except the functions that require accurate time like Pump Logging and Temperature Logging.  Next if we could set a flag so as soon as we have a valid ip address then time sync will be executed very quickly rather than waiting for the 10 minute interval we set and upon getting a valid time value for the RTC would automatically re-enable the logging functions. I'm now going to implement the instructions from your last post. 
...
It seems no matter what I do ever since switching from Arduino IDE 1.8x to Arduino IDE 2.x I cannot make the Ethernet connection truly stable with my ESP32-S3-DevkitC-1-N8R8 MCU running ESPAsyncWebServer. We've now spent well over 100 hours with countless revisions adding guardrails like  buffers and queue's and reconnection algorithms, task pinning and then unpinning, SPI busses isolation,  RTM based OneWire, Hardware SPI, and so many optimizations. The three instances where stability was noticeable improved were when we returned to non pinning the RTOS tasks and just letting FreRTOS load balance the cores and when I stopped reporting the File System usages and Free bytes and the biggest stability improvement was when I implemented the websocket queue so the system has a single sender for all outbound messages to the webpage, at least I'm pretty sure it's for all outbound WS messages and there are not still other functions that can directly send WS messages.  how do I get my entire code base to you so you can inspect everything in it's entirety so you can completly answer questions to assist me in troubleshooting the network disconnect errors that persist even after months of troubleshooting?
